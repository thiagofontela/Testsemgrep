...
# Mapeia level SARIF textual -> severidade numérica (melhora ordenação no GitHub)
...
def add_github_recognized_tags(rule_obj: dict, cwe_code: str | None, owasp_id: str | None) -> None:
    ...
    # CWE → external/cwe/cwe-89
    if cwe_code and cwe_code.upper().startswith("CWE-"):
        try:
            cwe_num = cwe_code.split("-", 1)[1].strip()
            tags.add(f"external/cwe/cwe-{cwe_num.lower()}")
        except Exception:
            pass

    # OWASP Top 10 2021 → external/owasp/2021/A03
    if owasp_id and "2021" in owasp_id:
        # exemplo "A03:2021 Injection" → A03
        left = owasp_id.split()[0]  # "A03:2021"
        a_code = left.split(":")[0] if ":" in left else left  # "A03"
...
def enrich(sarif: dict) -> dict:
    ...
    for run in runs:
        ...
        for r in results:
            ...
            # Enriquecimento específico por regra
            m = MAP.get(rule_id)
            if m:
                props["cwe"] = m.get("cwe")
            ...
            # Severidade numérica para melhor ordenação
